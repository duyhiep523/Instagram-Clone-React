...
import { reactToPost, unReactToPost, checkUserReaction } from '../../services/reactionService';
const PostModal = ({ onClose, post }) => {
    // Tất cả hook phải nằm ở đầu function
    const [detail, setDetail] = React.useState(null);
    const [loading, setLoading] = React.useState(false);
    const [currentIdx, setCurrentIdx] = React.useState(0);
    const [liked, setLiked] = React.useState(false);
    const [likeCount, setLikeCount] = React.useState(post?.likeCount || post?.likes || 0);
    ...
    React.useEffect(() => {
        ...
        checkUserReaction(currentUserId, post.postId || post.id)
            .then(res => { if (!ignore) setLiked(res); })
            .catch(() => { if (!ignore) setLiked(false); })
            .finally(() => { if (!ignore) setInitialLikeCheckLoading(false); });
        return () => { ignore = true; };
    }, [post, currentUserId]);
        ...
        <div className="modal-overlay" onClick={onClose}>
            <div className="modal-content post-modal-fixed-width" onClick={e => e.stopPropagation()}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: 24 }}>
                    ...
                    <div>
                        <div className="skeleton-image" style={{ width: 120, height: 16, marginBottom: 8 }}></div>
                        <div className="skeleton-image" style={{ width: 180, height: 14 }}></div>
                    </div>
                </div>
                <div className="skeleton-image" style={{ width: 300, height: 18, margin: '16px 24px' }}></div>
                <div style={{ padding: 40, textAlign: 'center' }}>Đang tải chi tiết bài viết...</div>
            </div>
        </div>
    );
            ...
            <div className="modal-overlay" onClick={onClose}>
                <div className="modal-content post-modal-fixed-width" onClick={(e) => e.stopPropagation()}>
                                ...
                                <div
                                    ...
                                    onDoubleClick={async e => {
                                        ...
                                        if (!liked) {
                                            ...
                                            setShowHeart(true);
                                            try {
                                                await reactToPost(currentUserId, post.postId || post.id);
                                            } catch (err) {
                                                setLiked(false);
                                                setLikeCount(count => count - 1);
                                            }
                    ...
                    <div className="post-details">
                        ...
                        <PostCaption caption={detail.caption || detail.content} createdAt={detail.createdAt} />
                        <CommentList
                        ...
                        <div className="small-interaction-container">
                                    ...
                                    {initialLikeCheckLoading ? (
                                        ...
                                        <div
                                            ...
                                            onClick={async () => {
                                                ...
                                                try {
                                                    if (prevLiked) {
                                                        await unReactToPost(currentUserId, post.postId || post.id);
                                                    } else {
                                                        await reactToPost(currentUserId, post.postId || post.id);
                                                    }
                                                } catch (err) {
                                                    ...
                                                    alert('Có lỗi xảy ra khi cập nhật trạng thái thả tim!');
                                                }
                                            }}
                                        >
                                            <FontAwesomeIcon
                                                icon={liked ? faSolidHeart : faRegularHeart}
                                                style={{ color: liked ? '#e1306c' : 'inherit', fontSize: 22, transition: 'color 0.18s' }}
                                            />
                                        </div>
                                    )}
                            ...
                            <div className="likes-time">
                                ...
                                <p>{typeof detail.commentCount !== 'undefined' ? detail.commentCount : (post.comments || 0)} bình luận</p>
                            </div>
